class LevelOne extends Phaser.Scene {
    constructor() {
        ...
        //create variable accessible by all levels
        window.GameState = {
            score: 0
        };
    ...
    preload() {
        ...
        this.load.image("enemyShipY", "enemyShipY.png");
        this.load.image("enemyShipG", "enemyShipG.png");
        this.load.image("playerBullet", "playerBullet.png");
        this.load.image("enemyBulletY", "enemyBulletY.png");

        //loading tilemap
        this.load.image("mapTiles", "tiles_packed.png");            // tile sheet   
        this.load.tilemapTiledJSON("map", "pixelShmupMap.json");    // Load JSON of tilemap
    ...
    create() {
        ...
        for (let i = 0; i < 7; i++) {
            let x = 100 + i * 120;
            let y = 100;
            my.sprite.enemyShips.push(new Enemy(this, x, y, "enemyShipY", null, 4, 2, 1, 80));
        }
        ...
        document.getElementById('description').innerHTML = '<h2>Level One.js</h2><br>A: left // D: right // Space: fire'
    ...
    update() {
        ...
        for (let enemy of my.sprite.enemyShips){
            if (enemy.cooldown < 0 && enemy.alive) { 
                this.bullet = new Bullet(this, enemy.x, enemy.y + 30, "enemyBulletY", null, -5, 2,)
                this.bullet.makeActive()
    ...
    endGame(status){
        ...
        if (status == "win") {
            this.add.bitmapText(game.config.width/2 - 225, game.config.height/2 + 50, "rocketSquare", "Press C to Continue", 35);
